...
local NvidiaRenderFolder = "NvidiaRender"
local ConfigurationFolder = NvidiaRenderFolder.."/Config"
local ConfigurationExtension = ".ncfr"

local NvidiaLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across Nvidia
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across Nvidia
...
local Nvidia = game:GetObjects("rbxassetid://10804731440")[1]

Nvidia.Enabled = false


if gethui then
	Nvidia.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(Nvidia)
	Nvidia.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	Nvidia.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	Nvidia.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Nvidia.Name and Interface ~= Nvidia then
			Interface.Enabled = false
			Interface.Name = "Nvidia-Old"
		end
	end
else
...
local Main = Nvidia.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

Nvidia.DisplayOrder = 100
...
local Notifications = Nvidia.Notifications

local SelectedTheme = NvidiaLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = NvidiaLibrary.Theme[ThemeName]
	for _, obj in ipairs(Nvidia:GetDescendants()) do
	...
	Nvidia.Main.BackgroundColor3 = SelectedTheme.Background
	Nvidia.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Nvidia.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Nvidia.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Nvidia.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Nvidia.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Nvidia.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if NvidiaLibrary.Flags[FlagName] then
			spawn(function() 
				if NvidiaLibrary.Flags[FlagName].Type == "ColorPicker" then
					NvidiaLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if NvidiaLibrary.Flags[FlagName].CurrentValue or NvidiaLibrary.Flags[FlagName].CurrentKeybind or NvidiaLibrary.Flags[FlagName].CurrentOption or NvidiaLibrary.Flags[FlagName].Color ~= FlagValue then NvidiaLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			NvidiaLibrary:Notify({Title = "Flag Error", Content = "Nvidia was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(NvidiaLibrary.Flags) do
...
function NvidiaLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= NvidiaLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("Nvidia | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
...
function Hide()
	Debounce = true
	NvidiaLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function NvidiaLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Nvidia Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "Nvidia Interface Suite" then
		LoadingFrame.Version.Text = "Nvidia UI"
	end
	...
	pcall(function()
		...
		if not isfolder(NvidiaRenderFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(NvidiaRenderFolder.."/Discord Invites") then
			makefolder(NvidiaRenderFolder.."/Discord Invites")
		end
		if not isfile(NvidiaRenderFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(NvidiaRenderFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Nvidia RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(NvidiaRenderFolder.."/Key System") then
			makefolder(NvidiaRenderFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Nvidia | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(NvidiaRenderFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(NvidiaRenderFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(NvidiaRenderFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						NvidiaLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Nvidia | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					NvidiaLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Nvidia | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("Nvidia | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					NvidiaLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Nvidia | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					NvidiaLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Nvidia | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					NvidiaLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Nvidia | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					NvidiaLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function NvidiaLibrary:Destroy()
	Nvidia:Destroy()
end
...
function NvidiaLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				NvidiaLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, NvidiaLibrary.LoadConfiguration, NvidiaLibrary)
if Nvidia:FindFirstChild("Notice") then
	Nvidia.Notice.Visible = true
	Nvidia.Notice.Interact.MouseButton1Click:Connect(function()
		Nvidia.Notice.Visible = false
	end)
end

return NvidiaLibrary